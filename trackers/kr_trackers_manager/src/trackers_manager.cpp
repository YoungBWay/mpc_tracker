...
#include <Eigen/Dense>
#include <kr_mav_msgs/MPCTrajState.h>
...
class TrackersManager : public nodelet::Nodelet
{
 ...
 private:
  ...
  // MPC communication test
  ros::Publisher to_mpc;
  kr_mav_msgs::MPCTrajState traj_state;
  kr_mav_msgs::MPCTrajState::ConstPtr traj_state_;
  int is_take_off = 0;
  double wait_for_20_points = 1;
};
...
void TrackersManager::onInit(void)
{
  ...
  // MPC communication test
  to_mpc = priv_nh.advertise<kr_mav_msgs::MPCTrajState>("to_mpc", 10);
}
...
//           if(msg->pose.pose.position.z >= 0.1999 && msg->pose.pose.position.z <= 0.201){
//             is_take_off = 1;
//           }
//         }
         pub_cmd_.publish(cmd_);
        ...
        kr_mav_msgs::SinglePoint point;
        point.header = cmd_->header;
        point.position = cmd_->position;
        point.time_from_start = cmd_->header.stamp.toSec();
        point.velocity = cmd_->velocity;
        point.acceleration = cmd_->acceleration;
        point.jerk = cmd_->jerk;
        point.yaw = cmd_->yaw;
        point.yaw_dot = cmd_->yaw_dot;
        Eigen::Quaterniond norm_orientation = Eigen::Quaterniond::Identity();
        point.orientation.w = norm_orientation.w();
        point.orientation.x = norm_orientation.x();
        point.orientation.y = norm_orientation.y();
        point.orientation.z = norm_orientation.z();
        point.bodyrates.x = 0;
        point.bodyrates.y = 0;
        point.bodyrates.z = 0;

        
        if(wait_for_20_points < 200){
          traj_state.header = cmd_->header;
          traj_state.points.push_back(point);
          wait_for_20_points++;
          // traj_state.state_posi = msg->pose.pose.position;
          // traj_state.state_vel = msg->twist.twist.linear;
          // traj_state.state_q = msg->pose.pose.orientation;
          // // MPC communication test
          // to_mpc.publish(traj_state);
        }
        else {
          if (wait_for_20_points == 200)
        {
          ...
          traj_state.points.push_back(point);
          wait_for_20_points++;
          traj_state.points.erase(traj_state.points.begin());
          traj_state.state_posi = msg->pose.pose.position;
          traj_state.state_vel = msg->twist.twist.linear;
          traj_state.state_q = msg->pose.pose.orientation;
          // MPC communication test
