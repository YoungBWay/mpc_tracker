...
kr_mav_msgs::PositionCommand::ConstPtr TrajectoryTracker::update(const nav_msgs::Odometry::ConstPtr &msg)
{
  ...
  // cmd->position.x = sin(traj_time), cmd->position.y = sin(2*traj_time), cmd->position.z = 0;



  cmd->position.x = sin(0.5*traj_time), cmd->position.y = cos(0.5*traj_time)-1, cmd->position.z = 0.2;
  cmd->yaw = 0;
  cmd->yaw_dot = 0;
  // cmd->velocity.x = cos(traj_time), cmd->velocity.y = cos(traj_time), cmd->velocity.z = 0;
  cmd->velocity.x = 0.5*cos(0.5*traj_time), cmd->velocity.y = -0.5*sin(0.5*traj_time), cmd->velocity.z = 0;
  cmd->acceleration.x = -0.25*sin(0.5*traj_time), cmd->acceleration.y = -0.25*cos(0.5*traj_time), cmd->acceleration.z = 0;
